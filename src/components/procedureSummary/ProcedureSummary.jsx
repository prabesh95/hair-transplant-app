...
const ProcedureSummary = () => {
  ...
  return (
    <div className='bg-soft-sage py-12' id='procedure-summary'>
    <div className="max-w-7xl mx-auto px-4 py-12" >
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <div className="lg:w-1/2">
          ...
          <div className="mt-8 bg-soft-sage/20 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-forest-green mb-3">Post-Procedure Care</h3>
            <ul className="space-y-2 text-stone-gray">
              <li className="flex items-start">
                <span className="text-olive-gold mr-2">•</span>
                <span>No heavy exercise for 1 week</span>
              </li>
              <li className="flex items-start">
                <span className="text-olive-gold mr-2">•</span>
                <span>Special shampoo after 48 hours</span>
              </li>
              <li className="flex items-start">
                <span className="text-olive-gold mr-2">•</span>
                <span>Sleep at 45° angle for first 3 nights</span>
              </li>
              <li className="flex items-start">
                <span className="text-olive-gold mr-2">•</span>
                <span>Follow-up appointments at 1 week, 1 month, and 3 months</span>
              </li>
            </ul>
          </div>
        </div>
          ...
          <div className="space-y-6">
            {steps.map((step, index) => (
              <div key={index} className="flex items-start">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-forest-green">{step.title}</h3>
                  <p className="text-stone-gray">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            {keyFacts.map((fact, index) => (
              <div key={index} className="flex items-center bg-white p-3 rounded-lg shadow-sm">
                <div className="text-olive-gold mr-2">{fact.icon}</div>
                <span className="text-stone-gray">{fact.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="my-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
        <h3 className="text-xl font-bold mb-3">Follow-Up Visits</h3>
      ...
      <div className="my-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <ul className="list-disc pl-5 text-gray-700">
          <li>Natural-looking hair growth in previously bald/thinning areas.</li>
          <li>Permanent results since transplanted hair is resistant to balding.</li>
        </ul>
      </div>
    </div>
    </div>
