...
const WhyChooseUs = () => {
  const features = [
    {
      icon: <CheckCircle className="text-olive-gold" size={24} />,
    ...
    {
      icon: <UserCheck className="text-olive-gold" size={24} />,
      title: "Personalized Care",
      description: "Custom treatment plans for each patient's unique needs"
    },
    {
      icon: <Clock className="text-olive-gold" size={24} />,
    ...
    <section className="py-12 ">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-forest-green">Why Choose Our Clinic?</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {features.map((feature, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-sm flex items-start">
              ...
              <div>
                <h3 className="text-xl font-semibold text-forest-green mb-2">{feature.title}</h3>
                <p className="text-stone-gray">{feature.description}</p>
              </div>
            </div>
          ))}
        </div>
